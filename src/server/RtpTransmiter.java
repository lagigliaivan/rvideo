...
ControllerListener{
	...
	public RtpTransmiter(MediaLocator locator,String ipAddress,	
						 String pb,int ptoDst,Format format) 
	{
	 ...
	 * 
	 * @return String que contiene el resultado de la inicializacion.
	 */
	...
	private String createTransmitter() {
		...
		for (int i = 0; i < pbss.length; i++) {
			try {
				...
				localAddr = new SessionAddress(getLocalAddress(),port);
				...
				logger.info( "Created RTP session: " + ipAddress + " " + port);
	...
	public synchronized void controllerUpdate(ControllerEvent ce) {
		logger.error("Controller update");
	}
				...
				if (owner instanceof Codec) {
					...
					for (int j = 0; j < fmts.length; j++) {
						if (fmts[j].matches(jpegFmt)) {
							...
							qc.setQuality(val);
							logger.error("- Setting quality to " + 
									val + " on " + qc);
							break;
						}
					}
