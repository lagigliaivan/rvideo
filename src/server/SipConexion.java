	...
	throws ExcGeneric{
		...
		try{
			...
			request.addHeader(getContactHeader());
                        getSipManager().getSipProvider().sendRequest(request);
			//clientTransaction = (SIPClientTransaction) getSipManager().getSipProvider().getNewClientTransaction(request);
			//clientTransaction.sendRequest();
			
		}
		catch (ParseException e){
			logger.error("error: messageFactory.createRequest" + e.getMessage() + e.getCause());
			throw new ExcGeneric("error:" + e.getCause() + e.getMessage());
			
		}
		catch (TransactionUnavailableException e){
			logger.error("error: sipProvider.getNewClientTransaction" + e.getMessage() + e.getCause());
			throw new ExcGeneric("error:" + e.getCause() + e.getMessage());
		}
		catch (SipException e){
			logger.error("error: clientTransaction.sendRequest" + e.getMessage() + e.getCause());
			throw new ExcGeneric("error:" + e.getCause() + e.getMessage());
		}
	...
	public void sendRegisterWithAutentic(Response response){
		...
		try{
			...
			registerRequestAuthen.addHeader(contactHeader);
                        getSipManager().getSipProvider().sendRequest(registerRequestAuthen);
...
//		Sip URI del agente que origina el request
	...
	public boolean setCSeq(int valor){
		...
		if(valor < 0){
			logger.error("error: No se puede setear el campo CSeq con un valor menor a 1");
			return false;
		}
		else{
			this.cSeq = valor;
