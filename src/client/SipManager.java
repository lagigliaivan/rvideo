...
public class SipManager implements SipListener	{
	 ...
	 * request
	 */
	public static final int VALOR_MAX_TIMEOUT = 40; 
	...
	private ThreadGroup threadGroup = new ThreadGroup("players");
	...
	public void sendInviteRequest(int indexConexion) throws ExcGeneric{
		 ...
		 */
		int reintentoError100 = 0;
		
		int reintentoError180 = 0;
		
		int reintentoError407 = 0;
		
		int reintentoError401 = 0;
		...
		while(true){
			...
			if(timeOut == VALOR_MAX_TIMEOUT){		
				...
				case 200:
					...
					ThrPlayer threadMediaPlayer = new ThrPlayer(dgSocketData,remoteAddress,
																remoteMediaPort,this,
																indexConexion,getThreadGroup(),
																getAllocator().getSipConexion(indexConexion).getToUser());
					 ...
					 */
					timeOut = 0;
					while(!threadMediaPlayer.isConected() && timeOut < VALOR_MAX_TIMEOUT){
						try {Thread.sleep(1000);} catch (InterruptedException e) {}
						timeOut++;
						//se conecto, por lo tanto salimos
					}
					if(timeOut == VALOR_MAX_TIMEOUT){
						throw new ExcGeneric("Se conecto con la webcam pero no se recivio el video");
					}
					else{
						return;	
					}
				...
				case 407:	
					...
					if(reintentoError407 < 5){
						...
						throw new ExcGeneric("407: El proxy necesita autenticacion");
					}
		...
		while(true){
			...
			if(timeOut == VALOR_MAX_TIMEOUT){		
				...
				case 407:
					...
					break;
				case 408:
					System.err.println("Nuestro servidor no contesta el bye pero desconecta :(");
					return;
				case 200:
					System.err.println("Nuestro servidor desconecto con exito :)");
					return;
				default:
					System.err.println("Se desconoce el codigo devuelto por el servidor");
					break;
				}
			}
		}
	...
	public void sendResponseToInvite(RequestEvent requestEvent) throws ExcGeneric{
		...
		try{
			...
			try {
				rtpTransmit.setLocalAddres(InetAddress.getByName(getStackAddress()));
	...
	public  void processRequest(RequestEvent event){
		...
		if (event.getRequest().getMethod().compareTo("BYE") == 0 && getRtpTransmit() != null){
			...
			try {
				...
				System.out.println("Error: Se produjo un error al contestar el BYE");
			}
			return;
		}else if(event.getRequest().getMethod().compareTo("INVITE") == 0){
				...
				return;
		}else if (event.getRequest().getMethod().compareTo("MESSAGE") == 0 ){
				threadList = new Thread[getThreadGroup().activeCount()];
				getThreadGroup().enumerate(threadList);
				String userRequest = event.getRequest().getHeader("From").toString();
				userRequest = userRequest.substring(userRequest.indexOf(':') + 1);
				userRequest = userRequest.substring(userRequest.indexOf(':') + 1, userRequest.indexOf('@'));
				
				for(int i = 0;i < threadList.length; i++){
					
					if(threadList[i].getName().compareTo(userRequest) == 0 ){
						try {
							sendOkResponse(event);
						} catch (ExcGeneric e) {
							System.out.println("Error: Se produjo un error al contestar el BYE");
...
class SipConexionAllocator {
	...
	public int allocateSipRequest(SipConexion sipConexion)throws ExcGeneric{
		...
		getVectorSipResponse().add(getVectorSipRequest().indexOf(sipConexion),null);
	...
	public Response getResponseForAt(int descriptorSipConexion)throws ExcGeneric{
		...
		/**
		 * si el descriptor es mayor al tamanio del vector o menor a 0 retorno una execpcion
		 */
		if(descriptorSipConexion >= getVectorSipResponse().size() || descriptorSipConexion < 0){
			...
			cleanSipResponse(descriptorSipConexion);
		}
		/**
...
class ThrBindRefresher extends Thread {
	...
	throws ExcGeneric
	{
		...
		if(manager != null && allocator != null){
			...
			throw new ExcGeneric("error: El par�metro sipconexion no debe ser null");
	}
	...
	public void run () {
		...
		while(true){
			try {
				...
				if(timeOut>= TIME_OUT){
					System.err.println("Error al refrescar el binding");
					break;
				}
				else{
					try{Thread.sleep(2000);}catch(InterruptedException excInterrupt){System.out.println("Error en el hilo refresher");};
					timeOut++; 
					continue;
				}
			}
			...
			if(expires > 4)  // paso de segundos a milisegundos y le resto 4 segundos
				expires = (expires * 1000) - 4000;
			else
				expires = 0;
			
			try{Thread.sleep(expires);}catch(InterruptedException excInterrupt){System.out.println("ERROR");};
			
			try {
				allocator.cleanSipResponse(sipConexionIndex);
			} catch (ExcGeneric excGeneric) {
				System.out.println(excGeneric.getMessage());
			}
			try {
				sipManager.sendRegisterRequest(sipConexionIndex);
			} catch (ExcGeneric e) {
				System.err.println("Error: Se produjo un error en el hilo que refresca el binding");
				return;
			}
		}
